...
| --- | --- | --- | --- | --- | --- |
| TCP | string | - | Conditional | ClickHouse TCP address list, comma-separated. If both `TCP` and `HTTP` are set, `TCP` takes priority. | `127.0.0.1:9000` |
| HTTP | string | - | Conditional | ClickHouse HTTP address list, comma-separated. Used only when `TCP` is not set. | `127.0.0.1:8123` |
| DB | string | - | Yes | ClickHouse database name. | `default` |
| UserName | string | - | Yes | ClickHouse username. | `default` |
...
| RecordTime | string | - | No | Timestamp mapping rule: `field` / `field,ns|us|ms|s` / `field,layout`. | `time,ms` |
| Tags | string | - | No | Tag segment mapping list, supports `ignore` and typed item `name|Type[|layout]`. | `ignore,server|String,id|Int32` |
| TagSplit | string | `.` | No | Delimiter used to split Fluent Bit tag before applying `Tags`. | `.` |
| Debug | bool | `false` | No | Enable clickhouse-go debug logging. | `true` |
...
| ReadTimeout | duration | `5m` | No | Read timeout for requests. | `1m` |
...
- `field`：直接将 Fluent Bit chunk timestamp 写入列 `field`。
- `field,ns|us|ms|s`：优先读取 record 字段 `field`，按单位解析；缺失或解析失败时回退到 Fluent Bit timestamp。
- `field,layout`：优先读取 record 字段 `field`，按 Go `layout` 解析；缺失或解析失败时回退到 Fluent Bit timestamp。

去重规则：如果 `field` 已经在 `Columns` 中出现，只会插入一次。
...
`Tags` 和 `TagSplit` 用于把 Fluent Bit tag 拆分后写入表列。

- `TagSplit`：tag 分隔符（默认 `.`）。
- `Tags`：拆分后每段的映射规则。
  - `ignore` 表示该段不写入。
  - `name|Type[|layout]` 表示该段按指定类型解析后写入。

最小示例：

```text
TagSplit=.
Tags=ignore,server|String,id|Int32
```

若 tag 为 `server.app.42`，则会写入 `server=app`、`id=42`。
...
3. `RecordTime` 使用了错误的 unit/layout
- 例：`time,mm` 或 layout 非法。
- 结果：配置期报错；或运行期解析失败后回退到 Fluent Bit timestamp。

4. `Tags` 与真实 tag 段数/分隔符不一致
- 例：`TagSplit=.` 但实际 tag 用 `-`。
- 结果：写入值错位，缺失段按默认值填充。

5. `HttpHeaders` 传入 `Host` 或 `Authorization`
- 结果：配置期校验失败（hard restriction）。

6. 仅配置 `TLSCert` 或仅配置 `TLSKey`
- 结果：配置期报错，必须成对设置。
