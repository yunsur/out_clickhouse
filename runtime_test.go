...
func TestClassifyInsertError(t *testing.T) {
	tests := []struct {
		...
		{name: "network reset", err: syscall.ECONNRESET, want: output.FLB_RETRY},
		{name: "network pipe", err: syscall.EPIPE, want: output.FLB_RETRY},
		...
		{name: "retryable clickhouse code 236", err: &clickhouse.Exception{Code: 236}, want: output.FLB_RETRY},
